        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <form onSubmit={handleSearchSubmit} className="flex items-center gap-4">
            <div className="flex-1 relative">
              <Search className="absolute left-5 top-1/2 transform -translate-y-1/2 h-5 w-5 text-[#0a4d2b] dark:text-[#F87171] z-10" strokeWidth={2.5} />
              <Input
                ref={inputRef}
                value={searchValue}
                onChange={(e) => onSearchChange(e.target.value)}
                placeholder="Search dishes or restaurants"
                className="pl-14 pr-16 h-13 w-full bg-white dark:bg-[#1a1a1a] border-gray-200 dark:border-gray-800 focus:ring-2 focus:ring-[#0a4d2b]/20 focus:border-[#0a4d2b] dark:focus:border-[#0a4d2b] rounded-2xl text-base dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 shadow-sm transition-all duration-200"
              />
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center gap-3">
                ...
                {isListening ? (
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 text-[#0a4d2b] dark:text-[#F87171] transition-all active:scale-90"
                  >
                    <Mic className="h-5 w-5" />
                  </button>
                )}
      ...
      <div className="flex-1 overflow-y-auto max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 scrollbar-hide bg-white dark:bg-[#0a0a0a]">
        ...
        >
          <h3 className="text-sm sm:text-base font-semibold text-gray-700 dark:text-gray-300 mb-4 flex items-center gap-2">
            <Clock className="h-4 w-4 text-[#0a4d2b]" />
            Recent Searches
          </h3>
              ...
              {filteredFoods.map((food, index) => (
                ...
                >
                  ...
                  <div className="px-1 sm:px-2 text-center">
                    <span className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-[#0a4d2b] dark:group-hover:text-orange-400 transition-colors line-clamp-2">
                      {food.name}
                    </span>
                  </div>
                </div>
