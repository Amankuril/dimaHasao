...
/**
 * The frame both admin auth screens sit in — sign in, and the password reset
 * it links to.
 *
 * Shared rather than copied so the two cannot drift: before this the login was
 * food-delivery red and the reset page was plain white, and clicking from one
 * to the other looked like leaving the product.
 *
...
export default function AdminAuthShell({ children }) {
  return (
    <div className="dh-admin-auth relative flex min-h-dvh w-full items-center justify-center overflow-hidden bg-[#04190c] px-4 py-8 sm:py-12">
      <style>{`
        .dh-admin-auth { font-family: 'Poppins', 'Montserrat', sans-serif; }
        .dh-admin-auth .dh-cinzel { font-family: 'Cinzel', serif; }
        .dh-admin-auth .dh-playfair { font-family: 'Playfair Display', serif; }
        .dh-admin-auth .dh-montserrat { font-family: 'Montserrat', sans-serif; }

        .dh-admin-auth .dh-weave {
        ...
        .dh-admin-auth input:-webkit-autofill,
        .dh-admin-auth input:-webkit-autofill:hover,
        .dh-admin-auth input:-webkit-autofill:focus {
      ...
      <div className="relative z-10 w-full max-w-[940px] overflow-hidden rounded-[26px] border border-[#caa83e]/30 bg-[#051f11]/95 shadow-[0_28px_80px_rgba(0,0,0,0.65)] backdrop-blur-md">
          ...
          <div className="relative hidden flex-col justify-between gap-8 border-r border-[#caa83e]/20 p-9 md:flex lg:p-11">
            <div>
              ...
              <p className="mt-6 max-w-[30ch] text-sm leading-relaxed text-[#cbd8cd]">
                The administration console for food, taxis, stays, tours and
                festivals across the district.
              </p>
            </div>

            <ul className="space-y-2.5">
              {["Food", "Taxi", "Hotels", "Tours", "Festivals"].map((service) => (
