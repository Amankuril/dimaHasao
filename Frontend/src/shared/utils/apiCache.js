/**
 * A small read-through cache for API calls, shared by every app in this repo.
 *
 * Nothing here had a cache. Every screen fetched on mount, unconditionally.
 * In the customer app, opening a stay and pressing back re-downloaded the whole
 * list. In the admin panel, one visit to the restaurants screen fires three
 * `limit=1000` requests, and every revisit fires them again. In a browser that
 * reads as a flicker; inside the Flutter wrappers, where a phone is doing the
 * work, it reads as the app being slow.
 *
 * This sits inside the service functions rather than in the screens, so callers
 * keep calling `fetchHotels()` or `adminAPI.getRestaurants()` and simply get an
 * answer immediately the second time.
 *
...
export const TTL = {
  ...
  MINE: 30 * 1000,
  /**
   * Operational data in a staff panel: an order queue, a rider's job list, a
   * partner's bookings. Deliberately short — the person looking at it is
   * acting on it — but long enough that moving between two screens and back
   * does not re-download it. Stale-while-revalidate does the real work here:
   * the screen paints from cache and corrects itself a moment later.
   */
  PANEL: 20 * 1000,
};
