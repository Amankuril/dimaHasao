...
export async function sendAdminResetOtpEmail(to, otp) {
    ...
    const subject = 'Your password reset code â€“ Dima Hasao Admin';
    const html = `
<!DOCTYPE html>
<html>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 480px; margin: 0 auto; padding: 20px;">
  ...
  <p style="color: #999; font-size: 12px;">Dima Hasao Admin</p>
</body>
</html>`;
    ...
    try {
        await trans.sendMail({
            from: typeof from === 'string' && from.includes('<') ? from : `Dima Hasao <${from}>`,
            to,
            subject,
            text,
            html
