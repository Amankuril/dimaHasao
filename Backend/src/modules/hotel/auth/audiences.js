 ...
 *
 * The three pieces below are exported because the combined partner login
 * (modules/partner) issues a hotel session alongside a restaurant one. Hotel
 * keeps owning its own model and rules; the other module borrows them rather
 ...
 */
export const signPartnerToken = (id, role) =>
    jwt.sign({ id, role }, process.env.JWT_ACCESS_SECRET, { expiresIn: '30d' });

/**
 * Create a partner from the minimum a login screen can ask for.
 *
 * Full KYC (Aadhaar, PAN, address, documents) is collected later in the partner
 * area — it needs file uploads, and gating signup behind it left new partners
 * with no way in at all. The account starts `pending`, so an admin still
 * approves before it can transact.
 *
 * Returns null when no name was given. verifyOtpForAudience calls createAccount
 * automatically whenever findAccount misses, so this is what stops a bare
 * verify from creating a half-built partner for someone who never asked for one.
 */
export const createHotelPartnerAccount = async (phone, { name, email } = {}) => {
...
/** Blocked or rejected partners may not sign in. Pending ones may. */
export const assertHotelPartnerCanLogin = (partner) => {
...
/** The partner half of a session: token, approval state and a trimmed user. */
export const issueHotelPartnerSession = async (partner) => {
    ...
    return {
        ...
        // Approval is separate from verification — the app uses this to show a
        // "pending approval" state instead of the dashboard.
...
export const registerHotelAuthAudiences = () => {
    registerAuthAudience({
        ...
        createAccount: createHotelPartnerAccount,
        assertCanLogin: assertHotelPartnerCanLogin,
        issueSession: issueHotelPartnerSession,
    });
};
